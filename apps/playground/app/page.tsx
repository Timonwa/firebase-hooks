    ...
    <div className="flex flex-col gap-6">
      <div>
        <h1 className="text-2xl font-semibold tracking-tight">
          Connected to{" "}
          <code className="font-mono text-accent">{config.projectId}</code>
        </h1>
          ...
          {STEPS.map((step, index) => (
            <li key={step.href}>
              <Link
                ...
                <span className="flex size-6 shrink-0 items-center justify-center rounded-full bg-accent/10 font-mono text-xs text-accent">
                  {index + 1}
                </span>
                <span>
      ...
      <div className="rounded-xl border border-line p-4 text-sm">
        ...
        <ul className="mt-3 flex flex-col gap-2 text-muted">
          <li>
            <strong>Authentication → Sign-in method</strong> — enable the
            providers you want to try. Email/Password covers most of the
            playground.
          </li>
          <li>
            <strong>Authentication → Templates → action URL</strong> — set it to{" "}
            <code className="font-mono">http://localhost:3000/auth/action</code>
            . Firebase uses its own hosted pages by default; this playground has
            its own at <code className="font-mono">/auth/action</code>, so
            overriding it lets you watch{" "}
            <code className="font-mono">useVerifyEmail</code> and{" "}
            <code className="font-mono">useConfirmPasswordReset</code> run here.
          </li>
          <li>
            <strong>Sign-in method → Phone</strong> — needed for{" "}
            <code className="font-mono">usePhoneSignIn</code>. Real numbers work
            and get a real SMS; registering a <strong>test number</strong> lets
            you verify with a code you pick instead.
          </li>
        </ul>
      </div>
    </div>
