...
export function UseLoginSection() {
  ...
  return (
    <HookSection
      ...
      why={
        <>
          By hand this is <code>useState</code> for loading and error,{" "}
          <code>try</code>/<code>catch</code>/<code>finally</code>, and the
          token exchange. The part that matters is that <code>onIdToken</code>{" "}
          runs <em>inside</em> the flow — throw in it and the sign-in aborts, so
          a user can't land on a protected page without a server session.
        </>
      }
      snippet={hookSnippet({
        hook: "useLogin",
