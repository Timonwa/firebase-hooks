    ...
    <HookSection
      ...
      why={
        <>
          ...
          <em>and returns the account email</em>, so the page can say whose
          password is being reset before asking for a new one — rather than
          accepting a new password and failing afterwards.
        </>
      }
...
await confirm(oobCode, newPassword);`}
      form={
        <div className="flex flex-col gap-2 text-sm text-muted">
          <p>
            This one needs a real <code>oobCode</code> from a reset email, so it
            lives on the action page Firebase links to.
          </p>
          <Link
            href="/auth/action"
