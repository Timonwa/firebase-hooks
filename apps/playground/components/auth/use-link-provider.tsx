...
export function UseLinkProviderSection() {
  ...
  return (
    <HookSection
      ...
      why={
        <>
          The guest-upgrade path. Sign in anonymously, then link — the uid stays
          the same, so everything created while anonymous still belongs to them.
          Without it you'd have to migrate their data to a new account.
        </>
      }
      snippet={hookSnippet({
        hook: "useLinkProvider",
      ...
      form={
        <>
          ...
          <Button
            ...
            disabled={isPending}
            onClick={async () =>
              setResult(await linkWithPassword(email, password))
            }>
