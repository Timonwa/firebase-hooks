  ...
  return (
    <div className="rounded-md border border-line">
      <p className="border-b border-line px-3 py-2 text-xs font-medium text-muted">
        What each <code className="font-mono">formatErrorMessage</code> would
        give for this failure. Set it in <strong>Options</strong> above, then
        run again.
      </p>
      <dl className="flex flex-col">
        {rows.map(row => {
          ...
          return (
            <div
              key={row.config}
              className="flex flex-col gap-1 border-b border-line px-3 py-2 last:border-b-0">
              <dt className="flex flex-wrap items-center gap-2">
                <code className="font-mono text-[11px] wrap-break-word text-muted">
                  {row.config}
                </code>
                {isApplied ? (
