...
There is no idle state — the work starts before you can render, so `pending` is where it begins. [`useVerifyEmail`](/docs/auth/use-verify-email) reports it as `VerifyEmailStatusType`, an alias of this.
