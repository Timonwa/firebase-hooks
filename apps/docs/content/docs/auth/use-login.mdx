...
`login(email, password)` resolves to a [`HookResult`](/docs/core/errors) — `{ success: true, user, credential }` on success, or `{ success: false, error, code, cause }` on failure. It never throws.
