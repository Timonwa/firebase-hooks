...
`status` is `"processing"`, `"success"`, or `"failed"` — exported as `VerifyEmailStatusType`, and the same [`AsyncStatus`](/docs/core/hook-result#asyncstatus) vocabulary every status in the package uses. `error` carries the message on failure.
