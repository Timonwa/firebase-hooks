...
`status` is `"pending"`, `"success"`, or `"error"` — exported as `VerifyEmailStatus`, and the same [`AsyncStatus`](/docs/core/hook-result#asyncstatus) vocabulary every status in the package uses. `error` carries the message on failure.
