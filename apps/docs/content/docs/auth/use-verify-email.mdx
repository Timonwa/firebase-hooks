...
```tsx
const { status, error } = useVerifyEmail(auth, searchParams.get("oobCode"), {
...
<AutoTypeTable path="../../packages/firebase-hooks/src/auth/use-verify-email.ts" name="UseVerifyEmailOptionsProps" />
