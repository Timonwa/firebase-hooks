...
`codeSent` flips true after a successful send. Also exposes `status`, `isPending`, `error`, and `reset`.
...
<AutoTypeTable
  path="../../packages/firebase-hooks/src/auth/use-phone-sign-in.ts"
  name="UsePhoneSignInOptions"
/>
